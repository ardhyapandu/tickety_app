<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            Event &raquo; {{ $event->name }} &raquo; Tiket
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="mb-10">
                <a href="{{ route('admin.events.tickets.create', $event->id) }}" class="px-4 py-2 font-bold text-white bg-green-500 rounded">
                    + Buat Tiket
                </a>
            </div>
                            ...
                            @foreach ($tickets as $ticket)
                                <tr class="border-b">
                                    ...
                                    <td class="px-6 py-4 space-y-1 text-center">
                                        ...
                                        <form 
                                            action="{{ route('admin.events.tickets.destroy', [
                                                'event' => $event->id,
                                                'ticket' => $ticket->id,
                                            ]) }}"
                                            method="POST"
                                            class="block"
                                            onsubmit="return
                                            confirm('Hapus tiket {{ $ticket->name }}?')"
                                        >
                                            @csrf
                                            @method('DELETE')
